      ...
      {list.length ? (
        list.map((item, index) => (
          <Item
            key={`item-${index}`}
            onClick={() => {
              setSelectedIndex(item.id, item);
            }}
            className={classnames('border border-whisper', {
        ...
        <div>
          <div className="center">
            <i
              className={`fa fa-${emptyListIcon} mb1`}
              style={{ fontSize: '5rem' }}
            />
          </div>
...
SelectableList.propTypes = {
  ...
  emptyListText: PropTypes.oneOfType([PropTypes.string, PropTypes.object]),
