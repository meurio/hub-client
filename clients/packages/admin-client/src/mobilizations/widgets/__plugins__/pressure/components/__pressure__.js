...
export class Pressure extends Component {
  ...
  componentWillMount() {
    const isPressurePhone =
      pressureHelper.getType(this.getTargetList()) ===
      pressureHelper.PRESSURE_TYPE_PHONE;
  ...
  handleOverlayOnClick(e) {
    ...
    if (editable) {
      ...
      history.push(paths.pressure(mobilization.id, widget.id));
    }
  }
  ...
  render() {
    ...
    return filledPressureWidgets.includes(widget.id) ||
